...
const PARTY_C = {
  ...
  TMC:{ fg:"#00695C", bg:"#E0F2F1" }, SP: { fg:"#D84315", bg:"#FBE9E7" },
  BSP:{ fg:"#1565C0", bg:"#E3F2FD" }, DMK:{ fg:"#C62828", bg:"#FFEBEE" },
  JDU:{ fg:"#00838F", bg:"#E0F7FA" }, RJD:{ fg:"#AD1457", bg:"#FCE4EC" },
  IND:{ fg:"#455A64", bg:"#ECEFF1" },
};
const TL_C = { party:"#6B21A8", legal:C.red, appt:C.orange, trade:C.blue, policy:"#444", gain:C.green, contract:C.orange };
const DIMS = [
...
// CORS proxy for Wikipedia images
...
const Sec = ({ children }) => (
  <div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:12}}>{children}</div>
);
...
function Ticker({ politicians }) {
  ...
  return (
    <div style={{background:"#0a0a0a",height:26,overflow:"hidden",
      ...
      <div style={{display:"flex",alignItems:"center",height:"100%",gap:10,padding:"0 10px"}}>
        ...
        <div style={{overflow:"hidden",flex:1,position:"relative",height:"100%"}}>
          <div style={{
            ...
            whiteSpace:"nowrap",fontSize:11,color:"#888",
            left:x,transition:"none",fontFamily:"monospace"
          }}>
            {doubled}
          </div>
        </div>
...
export default function NetaWatchClient({ initialData }) {
  ...
  const growthPct     = sel.totalAssets[earliestYear] && earliestYear!==latestYear
  ...
  const gap           = Math.max(0,(parseFloat(growthAbs)-parseFloat(totalDeclared))).toFixed(1);
  ...
  const partyStats = useMemo(()=>{
    ...
    return Object.entries(map)
      ...
      .sort((a,b)=>b.totalCases-a.totalCases).slice(0,12);
  },[initialData]);

  const totalCases = useMemo(()=>initialData.reduce((s,p)=>s+(p.criminalCases?.length||0),0),[initialData]);
  const highRisk   = useMemo(()=>initialData.filter(p=>p.scoring.final>=70).length,[initialData]);

  const TABS = [["overview","Overview"],["scoring","Score"],["network","Network"],["cases","Cases"],["party","Party"],["timeline","Timeline"]];
      ...
      <div className="layout">
        ...
        <aside className="sidebar" style={{background:C.card}}>
          <div style={{padding:"10px 12px 6px",borderBottom:`1px solid ${C.border}`}}>
            <input className="search" placeholder="Search name, party, state, constituency…"
          ...
          {filtered.length===0&&(
            <div style={{padding:"24px 16px",textAlign:"center",color:C.muted,fontSize:12}}>No results for "{search}"</div>
          )}
          <div style={{marginTop:"auto",padding:"12px 14px",borderTop:`1px solid ${C.border}`}}>
            <div style={{display:"flex",gap:12,marginBottom:8}}>
              <div style={{flex:1,textAlign:"center",background:C.redBg,borderRadius:5,padding:"8px 4px"}}>
                <div style={{fontSize:16,fontWeight:800,color:C.red}}>{totalCases}</div>
              ...
              <div style={{flex:1,textAlign:"center",background:C.orangeBg,borderRadius:5,padding:"8px 4px"}}>
                <div style={{fontSize:16,fontWeight:800,color:C.orange}}>{highRisk}</div>
                <div style={{fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:.5}}>High Risk</div>
              </div>
            </div>
            <p style={{fontSize:10,color:C.muted,lineHeight:1.7}}>
              Source: ECI · MCA21 · NJDG<br/>All data is public record.
            </p>
          </div>
        </aside>
        ...
        <main style={{overflowY:"auto",background:C.bg}}>
          ...
          <div style={{background:C.card,borderBottom:`1px solid ${C.border}`,padding:"16px 16px 0"}} className="fadein">
            <div style={{display:"flex",gap:14,alignItems:"flex-start",marginBottom:14}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:C.dim}}>
                  {[sel.constituency,sel.state,sel.age?`Age ${sel.age}`:null].filter(Boolean).join(" · ")}
                </div>
              </div>
            ...
            <div className="g5" style={{marginBottom:12}}>
              ...
              <StatCard label={growthPct?`Growth ${earliestYear}–${latestYear}`:"Declared (2024)"}
                ...
                sub={growthPct&&growthAbs>0?`+₹${growthAbs}Cr`:undefined} color={C.green}/>
              <StatCard label="Income Gap" value={`₹${gap}Cr`}
                color={parseFloat(gap)>5?C.red:parseFloat(gap)>2?C.orange:C.green}/>
            ...
            {parseFloat(gap)>0&&(
              <div style={{background:C.redBg,border:`1px solid ${C.red}33`,borderRadius:5,
                padding:"8px 12px",marginBottom:12,fontSize:12,color:C.red,fontWeight:600,lineHeight:1.6}}>
                ⚠ Assets: ₹{netWorth}Cr · Declared income on record: ₹{totalDeclared}Cr · Gap: ₹{gap}Cr unexplained.
              </div>
            )}
          ...
          <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:14}}>
            ...
            {tab==="overview"&&(<>
                ...
                <div className="card">
                  ...
                  {assetData.length>=2?(
                    ...
                    <div style={{height:180,display:"flex",alignItems:"center",justifyContent:"center",
                      flexDirection:"column",gap:8,color:C.muted,fontSize:12,textAlign:"center"}}>
                      <div style={{fontSize:32}}>📄</div>
                      <div>Only 2024 affidavit data available.<br/>Historical data added as scraper runs.</div>
                    </div>
                  )}
                </div>
                <div className="card">
                  ...
                  {sel.holdings?.length>0?(
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,alignItems:"center"}}>
                          ...
                          <div style={{display:"flex",gap:6,alignItems:"center"}}>
                            ...
                            <Badge label={h.conflict?"⚠ CONFLICT":"✓ CLEAR"}
                              color={h.conflict?C.red:C.green} bg={h.conflict?C.redBg:C.greenBg}/>
                          </div>
                        </div>
                    ...
                    <div style={{color:C.muted,fontSize:12,padding:"12px 0"}}>
                      Detailed holdings not yet parsed.<br/>Full data available after scraper enrichment.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="card">
                <Sec>Party-wise Criminal Cases — Top {partyStats.length} Parties</Sec>
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={partyStats}>
                    ...
                    <Bar dataKey="totalCases" name="Total Cases" radius={[3,3,0,0]}>
              ...
              {/* All politicians score chart */}
              <div className="card">
                <Sec>Suspicion Score — Top 30 of {initialData.length} Politicians</Sec>
              ...
              <div className="card" style={{background:C.redBg,border:`1px solid ${C.red}22`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
                  <div><Sec>Final Suspicion Score</Sec>
                    <div style={{fontSize:11,color:C.dim}}>Weighted average · Computed live from affidavit data</div>
                  </div>
                ...
                <div style={{borderTop:`1px solid ${C.red}22`,paddingTop:12,display:"flex",justifyContent:"flex-end",alignItems:"center",gap:8}}>
            ...
            {tab==="network"&&(
              <div className="card">
                ...
                {(!sel.network||sel.network.length===0)&&(
                  <div style={{color:C.muted,fontSize:12,padding:"12px 0"}}>No network data on record.</div>
                )}
                {sel.network?.map((n,i)=>{
            ...
            {tab==="cases"&&(
              <div className="card">
                ...
                {(!sel.criminalCases||sel.criminalCases.length===0)&&(
                  <div style={{color:C.green,fontSize:12,padding:"12px 0",fontWeight:600}}>✓ No criminal cases on record.</div>
                )}
            ...
            {tab==="party"&&(<>
              <div className="card">
                ...
                <ResponsiveContainer width="100%" height={150}>
                  <BarChart data={sel.partyHistory.map((p,i)=>({name:p.party,"Net Worth":Object.values(sel.totalAssets||{})[i]||0,fill:(PARTY_C[p.party]||PARTY_C.IND).fg}))}>
                    ...
                    <Tooltip content={<TT/>}/>
                    <Bar dataKey="Net Worth" radius={[3,3,0,0]}>{sel.partyHistory.map((p,i)=><Cell key={i} fill={(PARTY_C[p.party]||PARTY_C.IND).fg}/>)}</Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            {tab==="timeline"&&(
              <div className="card">
                ...
                {(!sel.timeline||sel.timeline.length===0)&&(
                  <div style={{color:C.muted,fontSize:12,padding:"12px 0"}}>Timeline data will be added after full enrichment.</div>
                )}
            ...
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14,fontSize:10,color:C.muted,lineHeight:1.7,textAlign:"center"}}>
              NetaWatch uses only publicly available data from the Election Commission of India, MCA21, NJDG, and Parliamentary records. Public interest transparency project.
            </div>
          </div>
        </main>
      </div>
